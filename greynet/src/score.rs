...
mod score_tests {
    ...
    #[test]
    fn test_simple_score_new_methods() {
        let mut score = SimpleScore::new(4.0);
        assert_eq!(score.get_fitness_value(), 0.8);

        let stub = SimpleScore::get_stub_score();
        assert_eq!(stub.simple_value, f64::INFINITY);

        let mut score_to_round = SimpleScore::new(3.14159);
        score_to_round.round(&[2]);
        assert_eq!(score_to_round.simple_value, 3.14);
    }

    #[test]
    fn test_hard_soft_score_new_methods() {
        let mut score = HardSoftScore::new(1.0, 3.0);
        // fitness = 0.5 * (1 - 1/2) + 0.5 * (1 - 1/4) = 0.5 * 0.5 + 0.5 * 0.75 = 0.25 + 0.375 = 0.625
        assert_eq!(score.get_fitness_value(), 0.625);

        let stub = HardSoftScore::get_stub_score();
        assert_eq!(stub.hard_score, f64::INFINITY);
        assert_eq!(stub.soft_score, f64::INFINITY);

        let mut score_to_round = HardSoftScore::new(1.234, 5.678);
        score_to_round.round(&[2, 1]);
        assert_eq!(score_to_round.hard_score, 1.23);
        assert_eq!(score_to_round.soft_score, 5.7);
    }

    #[test]
    fn test_hard_medium_soft_score_new_methods() {
        let mut score = HardMediumSoftScore::new(1.0, 2.0, 3.0);
        // fitness = (1/3)*(1-1/2) + (1/3)*(1-1/3) + (1/3)*(1-1/4)
        //         = (1/3)*0.5 + (1/3)*0.666... + (1/3)*0.75
        //         = (0.5 + 0.666... + 0.75) / 3 = 1.91666... / 3 = 0.63888...
        assert!((score.get_fitness_value() - 0.63888).abs() < 0.00001);

        let stub = HardMediumSoftScore::get_stub_score();
        assert_eq!(stub.hard_score, f64::INFINITY);
        assert_eq!(stub.medium_score, f64::INFINITY);
        assert_eq!(stub.soft_score, f64::INFINITY);

        let mut score_to_round = HardMediumSoftScore::new(1.111, 2.222, 3.333);
        score_to_round.round(&[1, 2, 0]);
        assert_eq!(score_to_round.hard_score, 1.1);
        assert_eq!(score_to_round.medium_score, 2.22);
        assert_eq!(score_to_round.soft_score, 3.0);
    }
}
